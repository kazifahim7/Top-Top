...
const updateLobbyInfo = async (id: string, payload: Record<string, unknown>) => {
     ...
     if (isLobbyExist.matchType === "teams" && payload.lobbyStatus === "completed") {
          ...
          if (goalTeam2 === 0) {
               const team1Goalkeeper = isLobbyExist.team1?.players?.find(
                    (p: any) => p.matchPosition === "Goalkeeper"
               );
               if (team1Goalkeeper) {
                    await userModel.findByIdAndUpdate(
                         team1Goalkeeper.playerId,
                         { $inc: { cleanSheet: 1 } }
                    );
               }
          }
          ...
          if (goalTeam1 === 0) {
               const team2Goalkeeper = isLobbyExist.team2?.players?.find(
                    (p: any) => p.matchPosition === "Goalkeeper"
               );
               if (team2Goalkeeper) {
                    await userModel.findByIdAndUpdate(
                         team2Goalkeeper.playerId,
                         { $inc: { cleanSheet: 1 } }
                    );
               }
          }
