...
async function addPlayerToLobby(payment: any): Promise<{ success: boolean; message: string }> {
     const lobby = await LobbyModel.findById(payment.lobbyId);
     if (!lobby) return { success: false, message: "Lobby not found" };

     if (!payment.playerId) return { success: false, message: "Player ID missing" };
     if (!payment.teamId) return { success: false, message: "Team ID missing" };
     ...
     const calculateAverageMainRating = (players: any[]) => {
          ...
          const sum = players.reduce((total: number, p: any) => total + (p.mainRating || p.rating || 0), 0);
     ...
     if (lobby.matchType === "solo") {
          ...
          if (requestedTeamId === lobby.defaultTeam1?._id?.toString()) {
               ...
               if (inTeam1) return { success: false, message: "Player already in team 1" };
               ...
               if (inTeam2) return { success: false, message: "Player already in team 2" };
