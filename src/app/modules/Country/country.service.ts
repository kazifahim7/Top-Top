...
const normalizeDialCode = (value: unknown): string => {
...
const normalizeCities = (value: unknown): CountryCity[] => {
     ...
     return value
          ...
          .filter((item): item is CountryCity => item !== null);
};

const ensureDefaultCountry = async () => {
     await CountryModel.updateOne(
          { countryCode: DEFAULT_COUNTRY_CODE },
          { $setOnInsert: DEFAULT_COUNTRY },
          { upsert: true }
     );
};
...
const getCountryByCode = async (countryCode: unknown, includeInactive = false) => {
     await ensureDefaultCountry();
     const normalizedCode = normalizeCountryCode(countryCode);
     const query: Record<string, unknown> = { countryCode: normalizedCode };
     if (!includeInactive) query.isActive = true;
     return CountryModel.findOne(query);
};

const assertActiveCountry = async (countryCode: unknown) => {
     const normalizedCode = normalizeCountryCode(countryCode);
...
const buildCountryPayload = (payload: Record<string, unknown>) => {
     ...
     if (!name) throw new AppError(400, "Country name is required");
     if (!dialCode) throw new AppError(400, "Dial code is required");
...
const createCountry = async (payload: Record<string, unknown>) => {
     await ensureDefaultCountry();
     const data = buildCountryPayload(payload);
     const existing = await CountryModel.findOne({ countryCode: data.countryCode });
     if (existing) throw new AppError(409, "Country already exists");
     return CountryModel.create(data);
};

const updateCountry = async (countryCode: string, payload: Record<string, unknown>) => {
     await ensureDefaultCountry();
     const normalizedCode = normalizeCountryCode(countryCode);
...
const deactivateCountry = async (countryCode: string) => {
...
const getAreasForCountry = async (countryCode: unknown) => {
     const country = await assertActiveCountry(countryCode);
     return country.cities;
};

const buildLegacyCountryFilter = (countryCode: unknown) => {
     const normalizedCode = normalizeCountryCode(countryCode);
