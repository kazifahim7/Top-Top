...
const parseMatchStartDate = (date: Date, time?: string) => {
     ...
     if (hours > 23 || minutes > 59) return matchDate;
...
const getHoursUntilMatchStart = (date: Date, time?: string) => {
     const matchStart = parseMatchStartDate(date, time);
     return (matchStart.getTime() - Date.now()) / (1000 * 60 * 60);
};

const playerExistsInLobby = (lobby: any, playerObjectId: Types.ObjectId) => {
     ...
     ].some((player: any) => player.playerId?.toString() === playerId);
};

const getPlayerTeamIdFromLobby = (lobby: any, playerObjectId: Types.ObjectId) => {
     const playerId = playerObjectId.toString();

     if (lobby.team1?.players?.some((player: any) => player.playerId?.toString() === playerId)) {
          return lobby.team1.teamId;
     }
     if (lobby.team2?.players?.some((player: any) => player.playerId?.toString() === playerId)) {
          return lobby.team2.teamId;
     }
     if (lobby.defaultTeam1?.players?.some((player: any) => player.playerId?.toString() === playerId)) {
          return lobby.defaultTeam1._id;
     }
     if (lobby.defaultTeam2?.players?.some((player: any) => player.playerId?.toString() === playerId)) {
...
const removePlayerFromLobby = async (
     lobbyObjectId: Types.ObjectId,
     playerObjectId: Types.ObjectId,
     session: ClientSession
) => {
     return LobbyModel.updateOne(
          { _id: lobbyObjectId },
          {
               $pull: {
...
const resetEmptyMatchFormats = async (lobbyObjectId: Types.ObjectId, session: ClientSession) => {
