...
function isSafeValue(value) {
    if (typeof value === 'string')
        return isSafeKey(value); // reject "$gt" as a value too
    if (typeof value === 'object' && value !== null)
        return false; // reject plain objects from qs
    return true;
}
function sanitiseQueryParams(raw) {
    const clean = {};
...
class QueryBuilder {
    ...
    query;
    allowedFilterFields;
    constructor(modelQuery, query, allowedFilterFields = []) {
    ...
    filter() {
        ...
        for (const key of this.allowedFilterFields) {
            if (META_PARAMS.has(key))
                continue; // never treat meta-params as filters
            if (!isSafeKey(key))
                continue; // defensive: allowlist itself must be safe
        ...
        if (this.query.organizer !== undefined) {
            ...
            if (!parsed.success)
                throw new Error('Invalid organizer format');
        ...
        if (!priceResult.success)
            throw new Error('Invalid price range parameters');
        ...
        if (this.query.playingDays !== undefined) {
            ...
            if (!parsed.success)
                throw new Error('Invalid playingDays format');
