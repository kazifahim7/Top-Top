...
export const isKnownFormation = (formation) => {
    return Object.prototype.hasOwnProperty.call(formationPositions, formation?.toString() || "");
};
export const normalizeMatchPosition = (position) => {
    const raw = position?.toString().trim() || "";
    if (!raw)
        return "";
    const upper = raw.toUpperCase();
    if (slotPositionNames[upper])
        return slotPositionNames[upper];
    return positionAliases[raw.toLowerCase()] || raw;
};
export const getFormationPositionNames = (formation) => {
    const slots = formationPositions[formation?.toString() || ""] || [];
    return slots.map((slot) => normalizeMatchPosition(slot));
};
export const getAllowedPositionCount = (formation, position) => {
    const requestedPosition = normalizeMatchPosition(position);
    if (!requestedPosition)
        return 0;
    const count = getFormationPositionNames(formation).filter((slotPosition) => slotPosition === requestedPosition).length;
    return count > 0 ? count : 1;
};
export const countPlayersInPosition = (players = [], position) => {
    const requestedPosition = normalizeMatchPosition(position);
    return players.filter((player) => normalizeMatchPosition(player?.matchPosition) === requestedPosition).length;
};
export const hasPositionCapacity = (players = [], formation, position, extraCount = 0) => {
...
export const remapPlayersToFormation = (players = [], formation) => {
    ...
    const findOpenSlot = (position, allowGoalkeeper = true) => {
        return slots.find((slot) => {
            if (slot.used)
                return false;
            if (!allowGoalkeeper && slot.position === "Goalkeeper")
                return false;
...
export const calculateAverageMainRating = (players = []) => {
    if (!players.length)
        return 0;
    const sum = players.reduce((total, player) => total + (player.mainRating || player.rating || 0), 0);
    return sum / players.length;
};
//# sourceMappingURL=lobbyFormation.js.map