...
export const getPlayerOverallRating = async (playerId) => {
    ...
    // ---------------- LOBBY MATCH ----------------
    const teams = ["team1", "team2", "defaultTeam1", "defaultTeam2"];
    ...
    lobbies.forEach(lobby => {
        ...
        for (const key of teams) {
            ...
            if (!team?.players?.length)
                continue;
            const player = team.players.find(p => p.playerId.toString() === playerId);
            if (player?.rating) {
