...
const createGlobalSetting = async (payload) => {
    const existing = await getGlobalSetting();
    if (existing)
        throw new AppError(409, "Transaction fee setting already exists");
    return TransactionFeeSettingModel.create({
        key: GLOBAL_KEY,
        percentage: normalizePercentage(payload.percentage),
    });
};
const updateGlobalSetting = async (payload) => {
    return TransactionFeeSettingModel.findOneAndUpdate({ key: GLOBAL_KEY }, { percentage: normalizePercentage(payload.percentage) }, { new: true, upsert: true, runValidators: true, setDefaultsOnInsert: true });
};
...
const updateCountryFixedFee = async (countryCode, payload) => {
    await CountryService.ensureDefaultCountry();
    ...
    const result = await CountryModel.findOneAndUpdate({ countryCode: normalizedCode }, { fixedTransactionFee }, { new: true, runValidators: true }).select("countryCode name currencyCode fixedTransactionFee isActive");
    if (!result)
        throw new AppError(404, "Country not found");
    return result;
};
const resolveQuoteSource = async (payload) => {
    const paymentType = payload.paymentType;
    if (paymentType === "team fee") {
        const lobbyId = payload.lobbyId;
        if (!lobbyId)
            throw new AppError(400, "lobbyId is required for team fee quotes");
    ...
    if (paymentType === "tournament fee") {
        ...
        if (!tournamentId)
            throw new AppError(400, "tournamentId is required for tournament fee quotes");
...
const buildQuote = async (payload) => {
...
//# sourceMappingURL=transactionFee.service.js.map