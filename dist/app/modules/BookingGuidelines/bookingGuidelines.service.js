...
const createGlobalGuidelines = async (payload) => {
    const existing = await BookingGuidelinesModel.findOne({ key: GLOBAL_KEY });
    if (existing)
        throw new AppError(409, "Booking guidelines already exist");
    return BookingGuidelinesModel.create({
        key: GLOBAL_KEY,
        content: normalizeContent(payload.content),
    });
};
const updateGlobalGuidelines = async (payload) => {
    return BookingGuidelinesModel.findOneAndUpdate({ key: GLOBAL_KEY }, { content: normalizeContent(payload.content) }, { new: true, upsert: true, runValidators: true, setDefaultsOnInsert: true });
};
...
//# sourceMappingURL=bookingGuidelines.service.js.map