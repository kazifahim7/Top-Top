...
import { countPlayersInPosition, getAllowedPositionCount, normalizeMatchPosition, } from "../../utils/lobbyFormation.js";
...
export const joinLobby = async (req, res) => {
    try {
        ...
        const normalizedMatchPosition = normalizeMatchPosition(matchPosition);
        let canonicalMatchFormat = matchFormat;
        ...
        if (teamPlayerId) {
            ...
            if (!isOrganizer && !isCaptain) {
                throw new AppError(403, "Only the team captain or organizer can add players on behalf of others");
            }
        }
        // ─── Lobby Validation ─────────────────────────────────────────────────
        if (lobbyId) {
            ...
            if (isLobbyExist.matchType === "solo") {
                ...
                if (!isOwner && teamPlayerId) {
                    throw new AppError(403, "Only the team captain can add players on behalf of others");
                }
            ...
            canonicalMatchFormat = currentTeam?.matchFormat || matchFormat;
            const playersInSamePosition = countPlayersInPosition(currentTeam?.players || [], normalizedMatchPosition);
            const allowedCountForPosition = getAllowedPositionCount(canonicalMatchFormat, normalizedMatchPosition);
            if (playersInSamePosition >= allowedCountForPosition) {
        ...
        const alreadyBooked = await PaymentModel.findOne({
            ...
            teamId,
            matchPosition: normalizedMatchPosition,
        ...
        if (paymentType === "team fee") {
            ...
            if (lobby.matchType === "teams") {
                ...
                if (isOwner && !teamPlayerId) {
                    ...
                    if ((lobby.team1?.teamId?.toString() === teamId?.toString() && team1Count >= lobby.teamSize) ||
                    ...
                    payment = await PaymentModel.create({
                        ...
                        method,
                        matchPosition: normalizedMatchPosition,
                        matchFormat: canonicalMatchFormat,
                        paymentType,
                        guest_player: isGuestPlayer,
                    });
                }
                ...
                const isDuplicate = (lobby.defaultTeam1?.players?.some((p) => p.playerId.toString() === playerObjectId.toString()) || false) ||
            ...
            if (tournament.type === "Knockout") {
                if (tournament.qualifiedTeams.length >= tournament.maxTeam)
                    throw new AppError(403, "Team is full");
...
async function checkPositionAvailability(payment) {
    ...
    const playersInPosition = countPlayersInPosition(targetTeamPlayers, requestedPosition);
    const allowedCountForPosition = getAllowedPositionCount(targetTeam?.matchFormat || payment.matchFormat, requestedPosition);
    ...
    const activePositionPaymentCount = positionPayments.filter((positionPayment) => {
        if (refundingPlayerIdSet.has(positionPayment.playerId?.toString() || ""))
            return false;
    ...
    if (playersInPosition + activePositionPaymentCount >= allowedCountForPosition) {
        ...
        if (pendingPaymentIds.length > 0) {
            await PaymentModel.updateMany({ _id: { $in: pendingPaymentIds } }, { $set: { status: "failed" } });
        }
    ...
    try {
        ...
        if (payment.paymentType === "team fee") {
            ...
            const calculateAverageMainRating = (players) => {
                if (!players || players.length === 0)
                    return 0;
                const sum = players.reduce((total, p) => {
        ...
        if (payment.paymentType === "tournament fee") {
            ...
            if (tournament.type === "League" || tournament.type === "Both") {
                result = await TournamentModel.findByIdAndUpdate(payment.tournamentId, { $addToSet: { teams: payment.teamId } }, { new: true });
            }
            else {
                result = await TournamentModel.findByIdAndUpdate(payment.tournamentId, { $addToSet: { qualifiedTeams: payment.teamId } }, { new: true });
            }
